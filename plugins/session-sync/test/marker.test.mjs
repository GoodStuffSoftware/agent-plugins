 ...
 * "Workstation-01" while PowerShell's $env:COMPUTERNAME wrote "WORKSTATION-01"
...
test('own marker is ignored despite differing case', () => {
  const marker = 'WORKSTATION-01|2026-08-30T20:45:51.000Z';
  assert.equal(parseMarker(marker, 'Workstation-01'), null,
...
test('a genuinely different machine IS detected', () => {
  ...
  const hit = parseMarker(marker, 'workstation-01');
...
test('a foreign push older than our last pull is ignored', () => {
  ...
  assert.equal(parseMarker(marker, 'workstation-01', '2026-08-30T12:00:00.000Z'), null,
...
test('a foreign push newer than our last pull is detected', () => {
  ...
  assert.ok(parseMarker(marker, 'workstation-01', '2026-08-30T12:00:00.000Z'));
});

test('malformed markers are ignored rather than throwing', () => {
  for (const bad of ['', 'garbage', 'no-pipe-here', 'host|not-a-date', '|', 'host|']) {
    assert.equal(parseMarker(bad, 'workstation-01'), null, `should ignore: ${JSON.stringify(bad)}`);
  }
});

test('surrounding whitespace does not defeat the self-check', () => {
  assert.equal(parseMarker('  WORKSTATION-01 |2026-08-30T20:45:51.000Z', 'workstation-01'), null);
});
