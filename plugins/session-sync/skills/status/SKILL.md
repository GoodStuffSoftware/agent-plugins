...
**When did it last run?** — from the log, `~/.claude/session-sync/last-pull.txt`, and `last-push.txt`. A last sync many hours old on a machine in daily use suggests hooks aren't firing: check the plugin is enabled and look for hook errors. A gap of a few minutes is normal and expected — hook-triggered pushes are debounced (`config.debounceMinutes`, default 5) so that several conversations ending close together coalesce into one sync instead of each triggering their own. Debounced pushes are **deferred, never dropped**: a background worker runs the collapsed push once the window lapses, and `~/.claude/session-sync/deferred-push.json` is the record that one is still owed (its presence during the window is normal; it should disappear once the push succeeds). A manual `push`/`pull` always runs immediately regardless.
